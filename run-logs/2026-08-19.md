...
**Concurrent-writer flag — re-verified, materially WORSE than 07-24 (9→8→57).**
The shared working tree now holds **57 dirty entries**, including **11 deletions** of files that are **live and committed in `main`**. Spot-verified live: `blog/2026-08-10-austin-housing-market-report-august-2026` → **200**, `updates/2026-08-10-…` → **200**, `assets/austin-neighborhood-august-2026.webp` → **200**. These are stale-checkout artifacts, **not** intentional removals. A `git add -A` in this tree would delete the August market report, its updates mirror, its hero image, 3 `scripts/*.mjs`, 3 `seo-data/*` files, 2 test files, and `netlify/functions/lib/lead-qualification.js` from the live site. I committed **only my 2 files by explicit path** and verified post-commit that the commit contained exactly those 2 (`git show --stat`). Left all 57 untouched.
...
## ISSUES FOUND
- **Concurrent-writer tree now 57 dirty entries incl. 11 deletions of LIVE files** — HIGH — escalated (was 8 modified on 07-24, no deletions). Real data-loss risk for any session that runs `git add -A`.
...
### Net new this run:
- **The shared working tree is now a data-loss hazard.** 57 dirty entries, and 11 of them are *deletions* of files that are live on the site and committed in `main` — including the August Austin market report (`blog/` + `updates/` + its hero image), 3 `scripts/*.mjs`, 3 `seo-data/*` files, 2 test files, and `netlify/functions/lib/lead-qualification.js`. Any session that runs `git add -A` or `git commit -a` in this repo will delete all of that from the live site in one commit. Worth having whichever session owns those edits either commit or reset them.
