...
second screenshot before logging a visual defect.

## 2026-09-04 — Thursday rotation (Internal Linking + Funnel Flow)

**A live-render-vs-source HTML diff is not evidence of a broken deploy by itself — check the
platform's own API for ground truth first.** Live-fetched HTML for `contact.html`,
`get-preapproved.html`, and `refinance-quote.html` was missing the bare `netlify` /
`netlify-honeypot="bot-field"` attributes that exist in the exact committed source at the
currently-deployed commit (confirmed via `git show <deploy-commit>:contact.html` against the
Netlify MCP's own `get-deploy-for-site` result — not assumed, the commit hash was cross-checked).
On its face this reads like a broken deploy silently dropping three of the site's highest-value
lead forms. Before writing that down, queried Netlify's Forms API via MCP
(`netlify-project-services-reader` → `get-forms-for-project`): all three forms are registered,
and `get-preapproved` had received a live submission **that same day**, hours before the run.
Netlify's build-time form detection appears to consume its own marker attributes from the served
HTML once a form is registered — consistent with the live HTML's reordered, single-quoted
attributes (a serialization signature of active processing, not a manual edit) and with every
tested form on the site showing the identical pattern. **Rule: when a live-render diff looks like
a platform-level failure, check the platform's own API before reporting it as one.** A curl diff
proves the served bytes differ from source; it does not prove which one is wrong, and for a
managed platform (Netlify Forms, in this case) the served bytes not matching source can be the
platform's own normal behavior.

**The Netlify MCP is a faster ground-truth path than inferring platform state from HTML.**
`netlify-project-services-reader` (`get-projects`, `get-forms-for-project`) and
`netlify-deploy-services-reader` (`get-deploy-for-site`) give direct read access to registered
forms (with submission counts and last-submission timestamps), current deploy state, commit ref,
and even a per-deploy Lighthouse plugin summary. Reach for these before building an HTML-based
proxy for something the platform already tracks authoritatively.

**When PSI is quota-drained, Netlify's own deploy record has a Lighthouse summary as a partial
substitute.** `get-deploy-for-site` on the current production deploy returned
`lighthouse_plugin_scores: "Summary for path '/': Performance: 80, Accessibility: 90, Best
Practices: 100, SEO: 100, PWA: 40"` — generated by Netlify's own Lighthouse plugin at build time,
zero cost, no quota. Scope is narrower than PSI (homepage only here, no confirmed per-page
