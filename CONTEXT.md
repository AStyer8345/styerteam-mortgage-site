...
3. **Resolve JS-bound events through `script.js`** — do NOT judge `/scenario.html`, `/prequal.html`, any `.js-quick-contact` form, or `phone_click` by page HTML alone. Page-level greps can't see the shared bundle (near-misses on 07-22 and 08-19).
4. **Do NOT re-flag the three 08-19 rejected findings:** the March-20 rates post's sitemap absence (correct — non-self-canonical), `loanos.html`'s missing meta (robots-Disallowed), and the `LocalBusiness` "9/25" gap (`MortgageBroker` is a subtype; actual coverage 25/25). Re-verify before re-surfacing.
