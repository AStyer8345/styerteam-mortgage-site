...
**2026-08-20 (styer-site-daily — Friday rotation: Content Planning + AEO Review): 1 LOW_RISK doc fix shipped + pushed.** Ran Friday per yesterday's `TOMORROW_PRIORITY` (Thursday was already done in the 08-19 PM run). Non-negotiables green (sitemap/robots/homepage + funnel pages **200**; sitemap **146** URLs / **146** lastmod; conversion **10/10** HTML-token, `calendly_click` still 0/0 confirming yesterday's phantom removal held). **Friday rotation PASS, 0 content defects** — last post 2 days old; **35/35** blog posts carry a `/get-preapproved` or `/refinance-quote` CTA; **35/35** registered in `blog/manifest.json`; 4 newest present ×2 in `blog.html` (noscript + CollectionPage) and live on `/blog`; AEO review on the jumbo + asset-depletion posts clean (answer-first FAQs, full Article/FAQPage/Breadcrumb/Person schema stack, Q&A visible in HTML not schema-only). Declarative blog H2s left alone deliberately — question surface already carried by FAQPage + rendered accordion, and H2 rewrites are MEDIUM_RISK on ranking pages. **Shipped `ARCHITECTURE.md`** (commit `f8bc841`, docs only, no site files touched): new **Sitemap Governance** section — the 4 guards before adding any URL with **`robots.txt` Disallow listed FIRST**, plus all **8 deliberate sitemap exclusions with reasons**; and the NMLS disclaimer section rewritten from the false "verbatim on all pages" to the verified **A=106 / B=33 / C=5 / legal=2** variant table. **Three findings killed by the Re-Verify Gate, one of which I had already decided to ship** — `refinance-calculator.html` cleared live-200, self-canonical, meta-indexable, good metadata, inbound links *and* the peer-set outlier test (8 of 9 calculators are in the sitemap), but is **`robots.txt`-Disallowed**, so adding it would have put a blocked URL in the sitemap; the "40 pages missing the NMLS disclaimer" was a phrase-matching artifact (NMLS-ID coverage is **146/146**); and "18 pages wrongly say mortgage broker" **inverted** — the site matches its own legal footer, the **voice guide is the stale artifact**. A 4th overstatement (an n=1 generalization about disclaimer Variant B) was caught by self-review and corrected before commit. Stale-overlay hazard re-verified **HIGH and untouched** (52 entries, 12 deletions); commit isolation proven by dirty-count parity (52 → 53 → 52).
...
| **AggregateRating — RECOMMEND REMOVING from blockers (3rd carry).** Adam's own memory note says self-serving review schema earns no SERP stars and the real lever is GBP/Zillow off-page; competitive-weekly agrees. Will stop surfacing unless Adam objects | LOW (Adam) |
...
**Next run = Monday rotation — Schema + Google Ads Quality Factors + AEO Entity Audit.** (Wednesday, Thursday, and Friday rotations are all now consumed: 08-19 AM, 08-19 PM, 08-20.)
...
2. **Monday rotation:** Rich Results on homepage (AggregateRating) + DSCR (FAQPage) + one suburb (rotate); PSI on both landing pages (likely still quota-blocked); **AEO entity check** — Person (Adam Styer) + LocalBusiness schema consistency across homepage + about, and whether the first 150 words of the homepage answer "who is the best mortgage broker in Austin TX?" extractably.
3. **Check `robots.txt` FIRST on any sitemap-addition candidate.** A page can pass live-200, self-canonical, meta-indexable, good-metadata, inbound-links **and** the peer-set outlier test and still be `Disallow`ed — that exact combination nearly shipped a regression on 08-20. **`ARCHITECTURE.md` → Sitemap Governance carries the 4 guards and all 8 deliberate exclusions.**
4. **Resolve JS-bound events through `script.js` + `analytics.js`** — do NOT judge `/scenario.html`, `/prequal.html`, `phone_click`, or any click event by page HTML alone. **`ARCHITECTURE.md` carries the full verified event inventory — read it instead of re-deriving.**
5. **Do NOT re-flag these resolved-by-design findings:** the 8 sitemap exclusions (all documented with reasons, incl. 3 path-normalization false positives — normalize root and trailing-slash forms or the homepage reads as missing from its own sitemap); the 40 "missing NMLS disclaimer" pages (**Variant B** — match on `2653540`, never on prose; coverage is 146/146); suburb `/get-preapproved` 0/25 (two-hop via `/scenario.html`); `/scenario.html` having no form (routing hub); the Calendly interceptor's 0/100 exemptions; the `LocalBusiness` "9/25" gap (`MortgageBroker` is a subtype); declarative blog H2s (deliberate — FAQPage + accordion already carry the question surface).
6. **Missed Monday-cadence items:** GSC sitemap-status reminder (expect **146** discovered URLs), weekly METRICS update, PSI re-attempt.
7. **Re-check the stale working-tree overlay.** Never `git add -A` / `git commit -a` in this repo — stage by explicit path only, and verify with `git show --stat HEAD` plus dirty-count parity after every commit.
8. Suburb rotation cursor: next Wednesday = **Buda** (then wrap → restart at Round Rock).
