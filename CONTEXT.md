...
| **NEW: Homepage 0 direct CTAs to `/get-preapproved` or `/refinance-quote`** post `71b8590` hero cleanup. **Partly addressed 2026-05-28 Q10:** hero CTA now routes to the on-site `#contact-form` quick form (in-house funnel, not raw 1003). Open: whether to also surface a direct `/get-preapproved` or `/refinance-quote` path. | MEDIUM (Adam) |
| **NEW 2026-05-28 Q10: Mobile sticky bottom-bar CTA** (`index.html` ~line 1167) still "Send My Scenario" → `/scenario.html` — the always-visible primary CTA on mobile still funnels to the 16-field form. Recommend repointing to `#contact-form` for mobile funnel consistency with the new hero CTA. One-line change. | MEDIUM (Adam) |
| **NEW 2026-05-28 Q10: n8n lead-alert drops loan goal.** Outlook "New Lead" email shows "Type: Not specified" because the quick form's field is `loanGoal` (camelCase) and the n8n Parse node only checks `loan_goal`/`loan_type`/`loan-type`. Goal IS captured in LoanOS via `lead-intake.js` — only the alert email is blind to it. Fix: add `loanGoal`/`loan-goal` fallback to the Parse node `loan_type` line, applied via REST PUT (per memory rule — MCP update_workflow wipes credential bindings). | MEDIUM (Adam) |
