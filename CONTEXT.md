...
5. **Older `<header>`-template AEO cluster — 2/16 closed; 14 remaining.** Audit method written + run on 20 posts. Pair cadence: ~~cash-out + fha-vs-conv~~ ✅ → va + ftb-programs (next) → dscr + how-to-choose-lender → self-employed + DPA → how-long-pre-approval + market-report → rate-volatility / life-devotional posts (separate pass at end).
