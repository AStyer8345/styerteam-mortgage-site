...
5. **Older-template AEO cluster — 4/16 closed; 12 remaining.** Pair cadence: ~~cash-out + fha-vs-conv~~ ✅ → ~~va + ftb-programs~~ ✅ → dscr + how-to-choose-lender (next) → self-employed + DPA → how-long-pre-approval + market-report → rate-volatility / life-devotional posts (separate pass at end).
