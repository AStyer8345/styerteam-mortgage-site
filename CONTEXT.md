...
2026-04-29 (daily-opt AM + PM) — **AM Wednesday rotation: Round Rock suburb deep dive + USDA cleanup + sitemap close.** Round Rock structural audit passed 15/15. Voice-guide compliance: removed USDA from 3 reader-facing/machine-extractable surfaces (LocalBusiness schema description, FAQPage schema "loan programs" answer, visible accordion); substituted DSCR + bank-statement language. dateModified + sitemap lastmod bumped to 2026-04-29. Sitemap: added rate-alert.html (closes prior LOW finding). Re-verify gate auto-resolved 2 stale flags (thank-you.html uncommitted 4-run HIGH; rate-alert sitemap). NEW consolidated flag: site-wide nav USDA dropdown (~88 pages — single Adam decision unblocks cascade). Commits ac042b4 → 6ff0f04 → 0db8c4c; live verified. **PM follow-up (TOMORROW_PRIORITY #5 advanced):** added `<p><strong>` answer-first paragraphs to 2026-03-20-austin-mortgage-rates-march-2026 (~67w, structured rate breakdown using post's own numbers) + 2026-03-27-down-payment-assistance-texas-2026 (~77w, distinct-angle TDHCA vs TSAHC comparison). Both used `<header>`/newsletter-author-bar insertion pattern. Sitemap lastmod bumped on both. AEO older-template cluster 6/16 → 8/16. Caught morning's TOMORROW_PRIORITY filename typo (how-to-take → how-long-does pre-approval). Commit 57e40d3; live verified via curl + grep. NotebookLM Step 0 dead 11th run.
