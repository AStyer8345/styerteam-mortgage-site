...
**2026-05-28 PM Q5–Q6 (user-initiated, Adam): Sitewide footer + scenario-CTA visibility fix, then cluster near-orphan rescue.** Q5: fixed invisible footer headings (34 drifted pages had `class="footer-col"` with zero CSS → navy-on-navy; renamed to canonical `.footer-section`) and the navy-on-navy "Have a Similar File?" scenario CTA (moved `</article>` above the CTA so `.bg-navy h2` white wins). Q6: rescued 2 near-orphan cluster variants via fresh-source related-list links (`non-qm-loans-self-employed-austin` 1→3 inbound, `bank-statement-loans-austin-tx` 2→3). Both detailed in CHANGELOG.

**2026-05-28 PM Q2/Q3/Q4 (user-initiated, Adam): SEO + scenario work.** Q2: link-graph audit of 147 pages + Tier-1 fix routing `/loans/*` product-page equity into clusters. Q3: Scenario #1 shipped live (`/scenarios/oil-gas-royalty-asset-depletion.html` — anonymized oil/gas-royalty asset-depletion story, hub card + lit chips, compliance-hedged). Q4: hub→spoke companion linking 4 cluster hubs to 4 under-equity'd complex-income spokes (1099-only, p-and-l, k1, business-owners). Audit Tiers 2–4 still pending (suburb→niche, blog→cluster, `/rates/index.html` 10-orphan rescue, FAQPage+AggregateRating audit, anchor-genericity). Full detail in CHANGELOG.
