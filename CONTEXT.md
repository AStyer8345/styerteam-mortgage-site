...
**2026-05-31 (subagent-driven build, supervised): styer-blog-writer-weekly task launched + first post published.** New scheduled SKILL task (`/Users/adamstyer/.claude/scheduled-tasks/styer-blog-writer-weekly/`, cron Tue 8am CT) that auto-publishes ONE net-new compliance-gated blog post/week — sibling to the Friday editor (which only refreshes existing posts; the writer is the only task that creates URLs). Guarantee: a post ships weekly unless a hard gate fails, and a failed week is announced loudly. First run executed inline & supervised: published **"Physician Mortgage Loans in Texas: How Doctors Qualify in 2026"** (`blog/2026-05-30-physician-mortgage-texas.html`, 1,387 words, Tier B). 4 approved-source citations (Fannie Mae B3-3.3-03 / B3-6-05 / B3-3.1-01, CFPB 1026.43); all 7 compliance gates PASS; registered across all 4 surfaces (blog.html noscript + CollectionPage, blog/manifest.json, sitemap.xml); 5 in-body cluster links. **Three commits:** `4876d86` publish (post + 4 surfaces, the clean revert target), `d850436` bookkeeping (run brief + backlog + changelog), `f7e14c5` added BreadcrumbList JSON-LD (post now ships Article+FAQPage+BreadcrumbList). Live verified HTTP 200 + BreadcrumbList present. SKILL.md fixes baked in during the run: manifest path (`blog/manifest.json` not root), sitemap `.html`-URL shape, two-commit flow (SHA chicken-and-egg), and schema-mirror now points at canonical 3-block template `2026-04-17-should-i-refinance-austin-tx-2026.html`. **To undo the post:** `git revert d850436..f7e14c5` (3-commit span; reverting `4876d86` alone now conflicts since `f7e14c5` touched the same file).

**2026-05-30 (user-initiated, Adam): Homepage restructure — hero lead form + section reorder.** Adam: "put the form in the hero, then loans built for every scenario below, then deals banks said no to below that." Built a **two-column hero** (`index.html`): left = pitch (rating chip, h1, subtitle, Book-15-Min-Call + call link, NMLS); right = new **compact 4-field lead form** (`id="hero-lead-form"`, `name="hero-quick-lead"`, Netlify Forms — Name/Phone/Email/Loan Goal + TCPA + "Get Started"). Desktop `.hero-cutout` photo hidden — form takes its place; columns stack ≤768px. New form gets `class="js-quick-contact"`; `script.js` `initFormValidation()` refactored to bind **all** `.js-quick-contact` forms (new `bindQuickContactForm()`), so hero + full form both run the existing dual pipeline (n8n Web Lead + lead-intake fn). **Section order now:** Hero → Loans Built for Every Situation → Deals Banks Said No To → full ~13-field quick-contact form (`#contact-form`, moved to intent peak per Adam's approved conversion choice). Deals CTA repointed `/scenario.html` → `#contact-form` ("Tell Me About Your File"). `style.css` got `.hero-glass-card-2col` flex rules; cache-busters → `?v=20260530`. Verified locally via Playwright (desktop 1280 two-column clean; mobile 390 stacks, no overflow clip, sticky bar intact). Committed + pushed; verified live (`40028bb`). **Follow-up same session:** added sitewide `generate_lead` GA4 conversion event — `dispatchLeadSubmitted()` in `script.js` now pushes `{event:'generate_lead', lead_type, form_name}` to the dataLayer (does NOT touch GTM snippet). Bumped every page's `script.js` cache-buster to `?v=20260530b` (102 HTML files) so returning visitors — esp. suburb Google Ads landing pages — refetch the new JS. **Still needed on GTM side:** a trigger on the `generate_lead` event → Google Ads conversion tag (site now emits the signal; container must consume it).

**2026-05-28 PM Q10 (user-initiated, Adam): Surfaced the homepage quick-capture form + added Purchase Price / Down Payment.** Adam wasn't getting leads because the hero's primary CTA funneled to the 16-field `scenario.html` while the friendly `#contact-form` quick form ("Tell Me About Your Loan", ~11 fields) sat buried near the bottom of the homepage. Repointed the hero CTA "Send My Scenario" → **"Get Started" (`#contact-form`)**, relocated the quick-form section to directly below the social-proof stats strip, added explicit **Purchase Price + Down Payment** inputs (kept the Loan Amount range), added a "Send a full scenario →" secondary link, and `scroll-margin-top:90px` so the anchor clears the sticky header. Single file (`index.html`, commit `089439a`), verified live. Backend needed zero edits — verified the live n8n Parse node already handles `purchase_price`/`down_payment`. **Two follow-ups (Adam approved "yes") now DONE & verified:** (1) mobile sticky bottom-bar CTA repointed "Send My Scenario"→`/scenario.html` to **"Get Started"→`#contact-form`** (commit `d2d2cea`, verified live) — every primary CTA now lands on the quick form; (2) n8n "Web Lead Automation" (`PiuIsQpBuydtFM4m`) Parse node patched via REST PUT to add `loanGoal`/`loan-goal` fallbacks so lead-alert emails no longer show "Type: Not specified" — verified live on published version (`versionId === activeVersionId`).
...
**2026-05-28 PM styer-site-daily (same-day second fire) — 0 mutations.** Per same-day-second-fire discipline (51st run cached): re-verify only, no copy-change piling. PM gate verified sitemap/conversion/legacy-scrub all green; AM findings hold; sitemap count moved 134 → 136 (Adam scenarios shipped). finding flagged for Adam batch: 147-file sitemap lastmod batch drift from Adam's 14-commit day (deferred to Friday AM batched sweep). [RESOLVED 2026-05-30: the `generate_lead` dataLayer-push gap is now fixed sitewide — see Last Worked On.] Refined carry: "complicated-income cluster 0 CTAs" → Adam funnel-direction implication (tracked LPs may be deprecation candidates given homepage-form-first PM direction). Full detail in `run-logs/2026-05-28-pm.md`.
