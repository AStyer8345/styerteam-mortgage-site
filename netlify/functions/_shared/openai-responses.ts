...
export async function createGeneralMortgageResponse(input: { message: string; conversation: Array<{ role: 'user' | 'assistant'; text: string }>; salesState: SalesConversationState; requiredQuestion: string }): Promise<SalesConversationResult> {
  ...
  const response = await fetch('https://api.openai.com/v1/responses', { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({
    ...
    instructions: `${SALES_CONVERSATION_DOCTRINE}\n\nAnswer first using reliable, broadly accepted mortgage knowledge. Explain concepts, tradeoffs, and typical process. Do not invent current rates, lender overlays, exact eligibility, approval, or personalized financial advice. Never create a hypothetical numerical example involving a rate, loan amount, payment, or cash to close unless those inputs came from the visitor and the application supplied the calculated result. Use confirmed context without assuming missing facts: ${safeDiscoveryContext(input.salesState)}. Ask no question except this exact director-selected question, and place it at the very end: ${input.requiredQuestion}. Never say you lack approved information, cannot bluff, or can only point them elsewhere. Do not recommend a calculator or CTA.`,
    input: [...recent, { role: 'user', content: [{ type: 'input_text', text: input.message }] }],
