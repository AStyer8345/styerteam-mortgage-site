(function(){
 ...
 function field(id){return document.getElementById(id+'-exact')||document.getElementById(id);}
 function snapshot(){var inputs={};Object.keys(ids).forEach(function(k){var input=field(ids[k]);if(input.getAttribute('aria-invalid')==='true')throw new Error('Correct the highlighted number before sending this calculation.');inputs[k]=Number(input.value.replace(/[$,]/g,''));});if(dscr)inputs.downMode=document.getElementById('dscr-down-pct-btn').classList.contains('is-active')?'pct':'dollar';return window.StyerAnalysis.normalize({kind:kind,at:Date.now(),inputs:inputs});}
 function restore(a){if(a.kind!==kind)return;Object.keys(ids).forEach(function(k){var input=field(ids[k]);input.value=a.inputs[k];input.dispatchEvent(new Event('input',{bubbles:true}));});if(dscr){document.getElementById(a.inputs.downMode==='pct'?'dscr-down-pct-btn':'dscr-down-dollar-btn').click();document.getElementById('dscr-down').value=a.inputs.down;document.getElementById('dscr-down').dispatchEvent(new Event('input',{bubbles:true}));}document.getElementById(dscr?'dscr-term':'new-term').dispatchEvent(new Event('change',{bubbles:true}));}
