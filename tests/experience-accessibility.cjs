...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});const findings=[];try{
 for(const width of [1440,390]){
  ...
  for(const route of ['/','/get-preapproved.html','/investor-loans.html','/dscr-calculator.html','/referral-partners-self-employed-clients.html','/bank-statement-loans.html','/scenario.html','/self-employed-mortgage-austin.html','/mortgage-for-business-owners-austin.html','/dscr-loan-austin-tx.html','/asset-depletion-mortgage-texas.html','/reverse-mortgage-texas.html','/k1-income-mortgage-austin.html','/high-net-worth-mortgage.html','/non-qm-loans.html','/contact.html']){
 ...
 assert.deepEqual(findings.filter(f=>f.violations.length),[]);console.log(findings.length+' desktop/mobile page scans: no automated WCAG A/AA violations detected.');
