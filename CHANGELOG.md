...
- **Correctness + de-template fix in the intro paragraph.** The bolded lead sentence cited "Arroyo Ranch" as a Dripping Springs neighborhood — it doesn't exist here (the page's actual acreage community is Arrowhead Ranch) and the sentence structure was the same templated skeleton found verbatim on the Liberty Hill and Manor pages ("...gives you access to 40+ wholesale lenders, same-day pre-approvals, and 1,000+ loans closed since 2017 — serving [county] buyers in [neighborhoods] since 2017," with "since 2017" doubled). Rewrote page-specific: correct neighborhoods (Headwaters, Caliterra, Sunset Canyon, Arrowhead Ranch), and led with the well/septic + jumbo angle that's actually differentiating here.
- **NEW: DSISD FY 2026-27 proposed tax rate**, added to the Property Tax section body, the FAQ JSON-LD, and the visible accordion. Board approved a $104.49M budget June 22, 2026 and is proposing $1.061/$100 ($0.711 M&O + $0.35 I&S) — about 4.4 cents below the current $1.1052 — but final adoption doesn't happen until the September 2026 board meetings, pending the state's compressed rate from TEA. Explicitly hedged as not-yet-final, and added the district's own finding that a median-valued home may still see a modest tax increase despite the lower rate, because rising valuations are outrunning the cut. Cited to [Community Impact](https://communityimpact.com/austin/southwest-austin-dripping-springs/education/2026/06/23/dripping-springs-isd-approves-104m-budget-proposes-lower-tax-rate/).
- **NEW: Double L Ranch (Anarene) reserves 200 acres for commercial/retail and land for a future DSISD campus** inside the development footprint — both facts from the original 2021 amended development agreement (City Council approved Sept 21, 2021) that still governs the 2026 buildout, per [Community Impact's contemporaneous reporting](https://communityimpact.com/austin/southwest-austin-dripping-springs/development/2021/09/10/dripping-springs-double-l-ranch-now-set-to-bring-2231-homes/). Did NOT publish a "250 senior living units" figure surfaced by a secondary real-estate blog — could not corroborate it against the City's own PZ staff report (fetched and text-extracted; that document covers only the 2021 Phase 1 preliminary plat and doesn't mention senior housing) or any news source, so it was cut rather than published unverified.
- **NEW: fresh Neuhaus Realty trailing-12-month print, dated August 2026** ($612,980 median / $269 per sqft / 67 DOM / 582 closings, pulled directly from Neuhaus's own MLS-fed market-data page) added alongside the existing April/May figures in both the body copy and FAQ/accordion — labeled explicitly as a rolling 12-month average, not compared apples-to-apples against the monthly Redfin/Neuhaus cuts already on the page, to avoid the methodology-conflation trap flagged in prior rounds. Also added the distinct $750,000 median **asking** price from a same-week Neuhaus post, clearly separated from sale-price data.
- Live Redfin fetch 403-blocked as documented (same pattern noted on prior suburb runs) — did not attempt to override the existing Redfin citations, only added the new Neuhaus print alongside them.
- `dateModified` 2026-07-03 → 2026-09-04 (WebPage schema) + sitemap `lastmod`. **4/4 JSON-LD blocks valid** (`json.loads`), tag balance clean (section 7/7, p 41/41, div 53/53, ul 14/14, li 99/99, h2 5/5, h3 7/7, h4 12/12), **5 FAQ questions / 5 accordion items in sync**, title lint clean, title 55 chars / meta description 148 chars (both within band, meta's "$510K median (Apr 2026)" label left as-is since it's still accurately dated and not the figure that changed). Rendered locally on the site's static dev server: HTTP 200, both new facts present, the "Arroyo Ranch" string returns 0 hits post-fix. **92/92 node tests pass.**
- Queue: Round 5 now 12/13. Next run: #13 westlake-mortgage-lender.html.
...
- **Mandatory citation pass: 0 → 5 authoritative external citations** on the niche-first queue's "do next" pick (verified live at 0 citations, 5 JSON-LD blocks, ~3,150 prose words before edit). (1) **Fannie Mae Selling Guide B2-2-03** (Multiple Financed Properties for the Same Borrower) — replaces a vague, unverifiable "guideline 5706" reference on the 10-property-cap claim with the real, current section. (2) **12 CFR § 1026.3 (Regulation Z, Exempt Transactions)** — new paragraph in the DSCR section sourcing *why* DSCR can skip personal DTI/ATR: non-owner-occupied rental credit is a business-purpose loan, exempt from the consumer Ability-to-Repay/QM rules. (3) **Fannie Mae B3-3.1-08** (rental income) — sources the pre-existing "75% of gross rent" underwriting line in the Conventional Investment section. (4) **Freddie Mac PMMS** — **6.71%** 30-yr benchmark week of Sept 3, 2026 (WebSearch-verified), anchoring the "0.5–1.0% below DSCR" rate-spread claim. (5) **Unlock MLS / Austin Board of Realtors** (via CultureMap, July 2026 report) — new section "Why Austin investors keep circling back": $435,000 metro median, 13,796 active listings, 63 days on market — first Austin data point on the page, framed as favorable BRRRR/DSCR conditions.
- **Enrichment 4/5:** Austin data point ✓ (new section above), current sourced rate data ✓ (Freddie Mac PMMS), hero alt text enriched ✓ (generic "Mortgage Broker for Real Estate Investors" → DSCR/portfolio/BRRRR specifics, no new asset), tighter positioning via the sourced 10-property-cap fix (removes an unverifiable "guideline 5706" claim that would have read as sloppy to a compliance reviewer). Anecdote skipped — `memory/people/` does not exist (checked repo + Documents root, consistent with 9 prior runs on this queue); would not fabricate.
- **All 5 citation URLs live-verified HTTP 200.** All 5 JSON-LD blocks re-validated (`json.loads`). Tag balance checked (p/div/section/ul/li/a all even). Blog title lint N/A (page isn't in `/blog/`) but title still contains "Adam Styer" ✓.
- **`dateModified` NOT bumped — the page carries no such field.** Unlike the blog-style niche pages this queue has run before (jumbo, DSCR, non-QM, etc.), `investor-loans.html`'s 5 schema blocks are `MortgageBroker` / `LoanOrCreditService` / `FAQPage` / `BreadcrumbList` / `Person` — none carries `datePublished`/`dateModified`. Did not fabricate a new schema block to hold one; that's outside the scope of a citation pass. Sitemap `lastmod` bumped 2026-05-18 → 2026-09-04 instead, matching the rest of the site's freshness signal.
- Title + H1 + slug preserved. Word count 3,150 → 3,479 (no padding — citations + one new sourced section).
...
- **Led with two correctness fixes, both in the closing-cost block.** (1) The page's cash-to-close itemization **did not sum to its own stated total**: the seven listed line items came to ~$14.1K–$15.6K while the headline claimed "roughly $20K–$25K." (2) The title line, "title insurance + endorsements ~$3,800," matches **no** TDI-promulgated figure — the owner's policy on a $1,000,000 purchase is **$5,226**, understated by ~$1,400. Rebuilt the whole block from [TDI Order 2025-9697](https://tdi.texas.gov/title/documents/titlerates2026.pdf) (eff. 2026-03-01), fetched and parsed directly rather than reused from prior-run precedent.
- **Differentiated insight — Bee Cave is the one page on this site where the second TDI bracket applies, and the marginal rate *falls* crossing $1M.** $100,001–$1,000,000 is (face − $100,000) × **0.00494** + $780; $1,000,001–$5,000,000 is (face − $1,000,000) × **0.00406** + $5,226. So a $1M policy is $5,226 and a $2M Spanish Oaks policy is **$9,286** — double the price, **78%** more premium. Verified the formula reproduces TDI's own Example 1 ($268,500 → $1,612) exactly before publishing.
- **Corrected the structure, not just the number:** in Travis County the owner's policy is **customarily the seller's** expense and is a negotiated blank at **TREC ¶6E**. Published both branches — customary split ≈ **$10,300–$11,700 plus the loan policy**; ¶6E flipped to the buyer ≈ **$15,500–$17,000**. Recording now cites Travis County's own **$25 first page / $4 each additional**. **Deliberately did NOT quote a simultaneous-issue loan-policy figure** — Order 2025-9697 is the basic premium schedule only and contains no rate rules, so the number was not verifiable at source and was cut rather than invented.
- **School correctness fix: the page asserted "38% math / 45% reading proficiency" for Lake Travis Elementary and gave no rating.** GreatSchools no longer publishes those percentages, so they were **cut as unverifiable**; direct fetch returned **6/10, 15303 Kollmeyer Dr, 78734, PK-5, 731 students**. Fixed in 3 places (body, FAQ schema, visible accordion) and reframed honestly — 6/10 is solid, but two points off Bee Cave Elementary, and that spread is what the Ladera zoning catch actually costs.
- **Material omission added — the Hamilton Pool Rd school that has not been built.** LTISD's **Elementary School #8** (K-5, 850 capacity, Child Development Center, adjacent to Provence) was slated for **August 2026**; the district's own [bond page](https://www.ltisdschools.org/bond) lists it **delayed and on hold pending enrollment**, no opening date, 16–18 months to build. Verified it did not open from a **primary negative**: LTISD's attendance-zone page still lists **seven** elementary campuses, independently corroborated by GreatSchools ("one of 7"). Published as a warning not to pay a school-zone premium on an unbuilt campus, and that its opening will **redraw boundaries** in both directions. Also corrected a stale news timeline: **Highland Lakes High School opens August 2029**, not the 2027/2028 the 2024 coverage reported — district page over article.
- **NEW neighborhood spotlight — Rocky Creek (78738)**, 0 prior hits sitewide: entrance on Rocky Creek Rd off Hamilton Pool Rd ~5 mi west of Hwy 71, built 2010–2019 by Drees/Highland (+ Taylor Morrison), 400+ homes, 2,367–5,910 sqft on 0.2–0.6-ac lots, 300 ac preserved green space, 5 mi trails. Financing angle stated as transparent arithmetic from two cited numbers: median plan is **4,012 sqft**, and $832,750 ÷ 4,012 = **$207.56/sqft** — the ceiling a home that size must stay under to finance conventionally, versus Bee Cave's cited **$293/sqft**. Conclusion: jumbo by default, and clearing the line takes ~25% down, not 20%.
- **NEW employer + the page's best differentiated insight, straight from Census data.** Added **Lake Travis ISD** as the corridor's uncounted major employer — **11 campuses, 11,008 students, 672.73 teachers, 1,393.91 total staff** (NCES, **2024-25** — used the primary source over a search snippet that mislabeled it 2025-26). Then the join: Bee Cave's employed population is **4,647** with a median household income of **$111,172** (Data USA, 2024 ACS) against a ~$1.0M median price — roughly **9:1 price-to-income**. Nobody qualifies for $1M on $111K of W-2 salary, and the occupation mix says why: Sales 836, Management 706, Business & Financial Ops 538; sectors Professional/Scientific/Technical 787, Finance & Insurance 527. A town of owners, partners and commissioned producers — i.e. K-1, 1099, distributions, RSUs. Directly serves the GOALS.md complicated-income positioning.
- **MEDIAN DELIBERATELY NOT UPDATED.** Sources were irreconcilable for the same city and month — ~$912,500 vs $1,095,000, +6.9% vs +38.6% YoY — and Redfin direct fetch returned **405** (documented block). Refused the swap and left the cited, vintage-labeled Redfin March 2026 figure with its May/June softening intact.
- **De-template:** exact-match grep across 24 suburb pages finds only footer and trust-strip matches, i.e. "clean." Normalizing city names and numerics before `difflib` exposed the **CTA subtitle verbatim on 7 pages** and a templated process intro. Both rewritten Bee Cave-specific (conforming line + income documentation). Rescan: the only remaining >0.60 matches are the shared footer contact block and the hero subtitle — both site design, not body copy. Body paragraphs **33 → 45**.
- **Also fixed:** the schools intro said "Four campuses" while the list now carries five — an inconsistency my own edit introduced, caught before commit.
- Verified: **4/4 JSON-LD valid**, FAQ **6 Q ↔ 6 accordion** in sync, tags balanced (section 7/7, p 52/52, li 78/78, div 56/56), **0 stale `2026-07-02`**, title 62 / meta 162 chars, `rel=noopener` on 100% of external anchors, **9/9 new source URLs HTTP 200**, **92/92 node tests pass** (`tsx` suite absent — pre-existing env gap). `dateModified` 2026-07-02 → 2026-08-31 + sitemap lastmod. Dirty-count parity 0 → 2 → 0.

## 2026-08-31 — styer-site-daily: 7-page referral cluster was live with no GTM container (coverage 146/153 → 153/153)

**Commit `a9d5188`** — 7 HTML pages, live-verified on production. Friday rotation (Content Planning + AEO Review).

- **Fixed the run's HIGH: seven indexed pages carried zero `GTM-PQQ6PGLR`** — the reverse-mortgage and financial-advisor/CPA referral cluster. Worse than missing analytics: `js/professional-referrals.js` calls `track('generate_lead')`, `track('partner_call_click')` and `track('professional_assistant_opened')`, each pushing into `window.dataLayer` — so the pages created the dataLayer and pushed conversions into it while no container ever loaded to consume them. Forms worked and Netlify captured leads, so nothing looked broken, but GA4 and Google Ads received zero conversion signal from the entire cluster. Added the standard synchronous snippet (head script + body noscript) copied byte-for-byte from `asset-depletion-mortgage-texas.html`, a peer in the same nav cluster. **+75/−12**; the "deletions" are re-emitted minified single-line files, proven by subtracting the inserted block and diffing against `HEAD` — all 7 byte-identical. Live-verified 7/7 at GTM ×2.
- **`git fetch` first, and it changed the run.** The checkout was **11 commits behind `origin/main`**, which made six brand-new pages look simultaneously like missing files, orphans, and dead ends — three "independent" checks naming the identical six URLs. `git pull --rebase` cleared all six. This task reads the **live** sitemap while analysing the **local** tree, so a lagging checkout manufactures HIGHs; fetching before analysis is now a precondition.
- **The NotebookLM advisor script was never missing — 103 runs reported it wrong.** It was archived to `Documents/cowork/archive/loanos-home-root/scripts/notebook_advisor.py`; the tell every prior run missed is that the whole *parent* directory was gone (a missing file is deletion; a missing home-root directory is a move). Confirmed it executes; **auth is the sole blocker** (`notebooklm login`).
- **Rotation came back clean and was generalized rather than sampled:** blog CTA coverage **59/59**, posts with an extractable summary in the first 3 paragraphs **59/59**, jumbo post FAQPage with **0 hedged answer openings**. Blog is **10 days stale** (last post 08-21 vs 7-day threshold) → flagged to `styer-content-weekly`. Standing metrics clean: orphans **0/153**, <2-outbound **0/153**, blog registry parity **3/3**.
- **Four of my own false findings killed before surfacing** — the stale-checkout six; "ItemList is empty" (regexed `"item":` when the schema uses `"url":`; real count 60); "all 59 posts missing from the noscript list" (`re.search` grabbed the GTM `<noscript>`, not the post list — `blog.html` has three); and "posts have no extractable summary" (a 2-post sample; all 59 checked, 0 defects). Two were uniform zeros from regexing structured data in a session that already had a correct extractor.
- **Corrected 3 SKILL.md drift items in-file** after 3 runs of flagging them with no resolution path (backup `SKILL.md.bak-2026-08-31`): dead advisor path, the retired "21-Day Avg. Close" trust-bar check inverted to verify absence per GOALS.md, and 2 pointers to the deprecated `styermortgage-context.md`. No logic, rotation, or hard constraint touched.

## 2026-08-30 — styer-suburb-editor-daily R5 #10: Lakeway — wrong price floor corrected, Hudson Bend feeder split, Tuscan Village added

**Commit `a099049`** — `lakeway-mortgage-lender.html` + `sitemap.xml`, verified live on production.

- **Led with a correctness fix, and it was load-bearing.** The page asserted "Homes here start around $600,000," which framed all of Lakeway as jumbo-only territory. It is wrong: [Tuscan Village](https://www.55places.com/texas/communities/tuscan-village), the 55+ community off Lohmans Crossing, currently starts in the **mid $400,000s**. Rewrote the intro to separate the single-family tier ($700K–$1.5M) from the attached 55+ tier and to make the borrower-facing point — two buyers in the same city can need completely different loans, and the sub-conforming buyer should not be steered into a jumbo product they don't need.
- **Page arrived 100% unique at paragraph level.** A full paragraph-level scan across all 24 other suburb pages returned **0 cross-page body duplicates** — only the shared, Adam-gated trust strip matched (on 12 pages, by design). De-template requirement therefore met per the established R4/R5 precedent by rewriting the most templated remaining passages, with similarity **measured before and after**: CTA subtitle **0.82 → 0.44**, process Step 3 **0.72 → out of the top 6**.
- **Four first-party cited additions.** (1) A **Tuscan Village** spotlight — Legend Communities, 196 planned homes, townhomes 1,294–1,790 sqft, HOA $295–$597/mo — carrying a financing angle no other suburb page has: HOA dues are underwriting-relevant (the top of that range consumes roughly the qualifying power of $100K of purchase price), attached product draws a project review detached homes never trigger, and downsizers here often want asset depletion rather than a debt-ratio approval. (2) **The two sides of town do not share a middle school** — the 78738 master-plans feed Lake Travis Middle while Lakeway Elementary on the Lohmans Crossing side feeds **Hudson Bend Middle** (8/10 GreatSchools, 806 students), both converging at Lake Travis High. (3) LTISD's **selective enrollment plan approved 2026-01-22** across 7 campuses, three of them on the Lakeway ladder. (4) **Lakeway Medical Village** — 53 acres, BSW-anchored, 100,000+ sqft of new medical space — tied to the complicated-income positioning, since that corridor staffs with practice owners on K-1s.
- **Replaced a speculative claim with a sourced one.** The page had been carrying an unsourced hedge that LTISD "will redraw attendance boundaries." The actual January 2026 board action tightens *out-of-district transfers* and explicitly does **not** change in-district boundaries — so the page now says that instead. This is a claim getting weaker and more accurate, not stronger.
- **Market data refreshed May → June 2026 from the same source.** Redfin $840,040/+4.7% → **$834,546/+7.7%**. Direct fetch was 405-blocked (the documented Redfin pattern), so the figure was confirmed by two independent searches before swapping. **The May sub-statistics were deliberately not carried onto the June headline** — June reports neither the closings count nor days-on-market, so the 244 closings and 98-day DOM remain explicitly labelled as the May cut. The closing-cost example was recomputed rather than one-number-swapped (loan $672,032 → $667,637; annual tax ~$15,500 → ~$15,400; escrow band $3,900–$5,200 → $3,900–$5,100). The $832,750 conforming limit was re-verified against FHFA and left alone.
- **One claim cut for lack of verification:** Tuscan Village's ZIP code. 55places lists its address in 78738; austinrealestatehomesblog places it off Lohmans Crossing in 78734, and Hudson Bend Middle — which serves it — is 78734. Sources conflict, so the page describes the community by street and omits the ZIP rather than pick one.
- 4/4 JSON-LD blocks valid; meta description 153 chars; tag balance and `rel=noopener` clean across all 38 external anchors; **66/66 node tests pass** (the `tsx` assistant suite could not run — the `tsx` binary is absent, a pre-existing environment gap unrelated to this change). Dirty-count parity 2 → 0.
- **Scheduler: 10-day outage** (08-20 Manor → 08-30 Lakeway), cross-checked against `git log` and run-log filenames before being claimed. Third recorded outage for this task; the trend is improving (22 days → 10) but daily firing still cannot be assumed.

## 2026-08-30 — styer-site-daily: three orphaned indexed pages linked into the site (orphans 3 → 0)

**Commit `ccff027`** — `blog.html` + `resources/index.html`, **+16 / −0** (zero deletions), live-verified.

- **Ran the complete version of the Thursday rotation rather than its 3-page sample:** a comment-aware inbound/outbound internal-link graph across all 147 sitemap URLs. The checklist's stated question — does each page link out to 2+ others — passes **147/147**, so every real defect sat on the **inbound** side the checklist never asks about.
- **Found 3 orphans — pages that are indexable, self-canonical, listed in `sitemap.xml`, and linked from nothing:** `/resources/mortgage-options-for-business-owners.html`, `/buy-before-you-sell-austin.html`, and `/resources/blog/2026-03-04-wait-for-rates.html`. Each was discoverable by Google only through the sitemap, receiving no internal link equity and reachable by no visitor. Checked the duplicate trap before fixing — an orphan that is a near-copy of a linked page needs a canonical, not a link — and all three came back distinct (0.43 and 0.56 body similarity, disjoint H2 sets, the third distinct by audience). **Added two resource cards to `/resources/` and registered the missing blog post; orphan count across indexed pages is now 0.**
- **Root cause of the third orphan was a three-registry desync in `blog.html`.** The post list is maintained in `blog/manifest.json` (drives the JS card grid), a `<noscript>` `<ul>` (carries the link equity), and CollectionPage `ItemList` schema — and the borrower "Wait for Rates" post was in the first only. All three held **59 entries**, so the counts matched while the contents differed; only a set diff exposed it. This is the rule already in `CLAUDE.md` ("New blog posts must be added to `blog.html` noscript block + CollectionPage schema") and one post had slipped it.
- **Funnel traced clean end to end.** Homepage → `/get-preapproved` (×5, as `?intent=scenario`) → Netlify form → `/thank-you`; all three forms correctly wired (`netlify` attribute, `form-name` hidden input, `bot-field` honeypot, `/thank-you` action, TCPA copy); `thank-you.html` carries Calendly ×2, a `tel:` link, `thank_you_page_view` ×1, the 3-step what-happens-next, and correct `noindex` plus sitemap absence. Conversion tracking **10/10**, GTM **147/147**, 0 sitemap URLs carrying `noindex`.
- **Two of the agent's own false findings killed before surfacing, both extractor bugs rather than site defects:** (1) a "dangling `/scenarios/[slug].html` placeholder" that is real and 404s live but sits **inside an HTML comment** — the whole graph was then rebuilt comment-aware and re-diffed, because comments can equally manufacture phantom *inbound* links and hide real orphans; (2) "the homepage has 0 links to `/get-preapproved`" — the regex anchored immediately after the path and missed `?intent=scenario`, where the true count is **5**. Both came from a second, sloppier inline regex written when a correct normalizing extractor already existed in the same session.
...
- **Correctness led this run; three live claims were wrong, two of them load-bearing.**
- **(1) The Whisper Valley high school is not opening. The page said "opens Fall 2026" — it is now Fall 2028.** The campus is **North Del Valle High School**, and it slipped twice: August 2026 → Fall 2027 → **Fall 2028**, announced 2026-05-27. Cause is not construction — the building is on schedule — but the developer's unbuilt access roads: DVISD states "the developer is now in default of its obligations under the Post-Closing Construction Agreement," with **$12.3M+** of roadwork outstanding ([DVISD](https://www.dvisd.net/departments/communications/north-del-valle-hs-updates); [FOX 7](https://www.fox7austin.com/news/del-valle-isd-pushes-back-start-date-new-high-school)). Verified independently at the district source before editing. The bad claim traced to a **December 2022** Whisper Valley marketing page still saying "August of 2026" — that citation is now removed from the page. Corrected in **4 places incl. FAQPage JSON-LD**.
- **(2) Lagos Elementary is 3/10, not 6/10 — and the page's school argument was built on the wrong number.** It called Lagos "the strongest-rated elementary among the named communities" and claimed its math scores ran "roughly three times higher" than ShadowGlen's. GreatSchools reads **3/10** today ("performing below average"), against ShadowGlen's 2/10 — a one-point gap, not a tier gap. Re-verified directly at source. Rewrote the section around the honest finding: **there is no elementary arbitrage in Manor ISD** — ShadowGlen 2/10, Lagos 3/10, Presidential Meadows 3/10, Manor Middle 2/10 all cluster. New organizing insight: **the only meaningful rating spread is the lottery, not the address** (Manor HS 2/10 by zoning vs Manor New Tech 4/10 by application), so budget on payment and taxes and treat the high school as an application to prepare for.
- **(3) The page double-counted Tesla's layoffs, overstating the contraction by ~4,700 jobs.** It reported the 2025 decline *and* "another ~22% reduction announced in early 2026." There was no second round — the 22% decline occurred during calendar 2025 and was merely *reported* in April 2026 when a Travis County compliance filing surfaced ([TechCrunch](https://techcrunch.com/2026/04/03/teslas-texas-factory-workforce-reportedly-shrunk-22-in-2025/)). Corrected in **4 places incl. FAQPage JSON-LD**, and noted Tesla has since been posting Giga Texas Cybercab manufacturing roles. Replaced the two 403-blocked citations (KXAN, TechBuzz) with a fetchable primary-adjacent source.
- **Differentiated insight — the ShadowGlen MUD number, pulled from the statutory disclosure the page tells buyers to demand.** Two rounds of this page said only "ShadowGlen MUD adds a similar overlay" with no figure. Also corrected the **spelling**: it is **Wilbarger** Creek, not "Wilberger" (the misspelling sends buyers to dead search results; 0 prior sitewide hits for the correct name). **Wilbarger Creek MUD No. 1's Notice to Purchaser discloses $0.7720/$100** — **72% as large as the entire school district rate**, ≈**$2,625/yr / $219/mo** on a $340K valuation, ≈**$34,600 of borrowing power** at 6.5%/30yr. **The better find is further down the same notice:** the district has voter approval for **$29.9M** water/sewer/drainage bonds but has issued only **$16.18M**, plus **$4.985M** park bonds with **none issued** — ≈**$18.7M authorized-but-unissued against $16.18M issued** — and the notice states it "may impose an unlimited rate of tax" to pay them. Published as: this rate is not a ceiling, do not underwrite 30 years against it. Also documented that MUD No. 1 lies **wholly in Manor's ETJ**, so the MUD is charged *instead of* City of Manor ad valorem, not on top.
- **Second differentiated insight — the captive-lender problem, stated against interest.** Both cited builder incentives were wrong. D.R. Horton's "$25,000 flex cash through Dec 31 2026" **appears nowhere on the live page** (0 occurrences of "flex cash" or "25,000"); the actual offer is a **2/1 buydown, $6,497.01, 2.99%/3.99%/4.99%, contracts 07/10/26 closing by 09/30/26, requiring a 0.375% discount point**. Pacesetter's "4.99% through May 16 2026" at Whisper Valley is **gone entirely**. Both replaced with what is actually advertised, plus the part no competitor prints: **these incentives require DHI Mortgage / KBHS Home Loans, so no outside broker — Adam included — can deliver them.** Framed honestly as: an outside pre-approval doesn't beat the offer, it *prices* it. Also flagged KB Home's "4.75%" headline as a **5/1 ARM** with a $5,000 credit baked into APR, expiring 8/31/26. New **6th FAQ** ("Should I use the builder's lender?") mirrored in schema + accordion.
- **Reclassified Mustang Crossing from employer to empty building.** The page sold it as a growth story. Phase 1 (582,080 sqft) delivered on schedule per [TDLR](https://www.tdlr.texas.gov/TABS/Search/Print/TABS2024024365) and is **100% vacant with no named tenant**; ~**4.87M sqft** of Manor industrial space is listed with none either. Likewise reframed OPmobility: its "400" and "800" figures are a 2024 projection and a 2023 site-selection announcement, **never a payroll count** — plant is operating and hiring, no TWC WARN filed, so the page now says "several hundred" and calls precise numbers marketing.
- **2 new subdivisions, both pinned to real addresses, 0 prior sitewide hits** — **Village at Manor Commons** (12713 Skimmer Run, Pacesetter **$329,900–$411,900**, HOA **$400/yr** with the billing period stated, Chesmar section sold out Apr 2025, confirmed in [City of Manor permit records](https://www.manortx.gov/Archive/ViewFile/Item/189); published its **conflicting elementary assignment** — Pacesetter says Lagos, aggregators say Presidential Meadows — as a verify-with-district item rather than picking one) and **Stonewater** (Pernella Rd / Stoneridge Gap Ln, built out 2008–2018, resale **$210,000–$475,000**, HOA **~$117–$129/quarter**, feeds ShadowGlen Elem). Deliberately **cut Manor Heights** despite 23 permits in one month — five permits label it "Carillon/Manor Heights," so it may be a section of a community already on the page. Also cut **Applied Materials** (it's Austin 78724; job boards mis-tag it as Manor) and a claimed east-Travis data center (unverifiable).
- **2 new employers** — **St. David's Emergency Center** (10703 E US-290, opened **2025-12-03**, 10,840 sqft, $15.3M, ~25 staff), which ended Manor's status as a town of 20,000+ with **no ER**; and **TXM MicroHospital** (12838 US-290, $25M) labeled **under construction, zero jobs**. Added **Home Depot #6541** (open May 2025) alongside H-E-B #811 (opened **2025-10-22**), and **trimmed the Manor Crossing tenant list to the four verifiable as open** — the rest were leasing-brochure logos presented as tenants.
- **Material omission added: Manor ISD has a $400M bond on the November 3, 2026 ballot** (Prop A $391.5M + Prop B $8.5M, called 6-0 on 2026-08-10), projected to add **3.85 cents/$100**. The page previously said bond construction was "on hold." Quantified honestly: **$130.90/yr on a $340K valuation, ~$92/yr** once the $100K school homestead exemption applies — and noted a first-year purchase doesn't have it yet. **Caught and fixed my own arithmetic error here** (first wrote $109/yr; 3,400 × $0.0385 = $130.90).
- **Also corrected:** Manor ISD rate **$1.0814 → $1.0755** adopted 2026-08-17 (M&O 0.7054 + I&S 0.3701), published *without* calling it a tax cut since the minutes carry the required "raises more M&O revenue" notice; **campus count 15 vs 16 contradicted itself on the same page** → 16 per district directory + NCES, with the Texas Tribune discrepancy explained; enrollment relabeled **2025-26** (2026-27 doesn't exist until the October PEIMS snapshot); Decker→Oak Meadows boundary shift corrected from "has weighed" to **adopted, effective 2024-25**; Manor HS enrollment 2,422 → **2,420**; Samsung "early 2027" → **2027**. **Two broken GreatSchools links fixed** — the Del Valle HS link (ID 4569) actually served **Manor Middle School**, and the Gilbert Elementary link resolved to a city page.
- **Cut rather than shipped:** all four math/reading proficiency percentages (source pulled **math from 2023 and reading from 2022** — different years, and its ShadowGlen math was 31%, not the 21% on the page); any mid-2026 Tesla headcount (the county report is annual, next lands ~April 2027); Austin industrial vacancy ~22% (every market report paywalled); North Del Valle HS street address (never published).
- **Median deliberately NOT updated.** Redfin and HAR both 403 automated fetch, and the only newer figure available was a **median list price ($374K, June)** — a different metric from the sourced **$340,000 median sale price (March 2026)**. Refused to swap one for the other; instead labeled the March figure's vintage on-page and told buyers to pull a current CMA. Flagged for next run.
- **De-templating: 2 paragraphs, both confirmed by normalized scan.** Exact-match grep is insufficient here; normalizing city names and numerics before `difflib` exposed **Step 2 verbatim on 5 pages** (bastrop, kyle, new-braunfels, san-marcos, taylor) and the **hero subtitle on 9**. Both rewritten Manor-specific — Step 2 now covers the **MUD/PID notice, HOA resale certificate (with the monthly-vs-quarterly-vs-annual trap), and DPA paperwork timing**. Post-edit rescan: those 2 gone, body blocks **78 → 84**.
- **Closing costs rebuilt from primary sources, replacing an uncited estimate.** Verified the TDI formula **against TDI's own worked example** rather than reusing prior-run precedent: [Order 2025-9697](https://tdi.texas.gov/title/documents/titlerates2026.pdf) eff. 2026-03-01 (6.2% reduction), **(face − $100,000) × 0.00494 + $780**; TDI's Example 1 ($268,500 → $1,612) reproduces exactly. Two worked examples: **Village at Manor Commons FHA 3.5% at $329,900** (title **$1,916**, ~$18,750–$20,500 to the table) and **Stonewater at $210,000** (title **$1,323**, ~$13,500–$15,250). Recording at Travis County's **published $25 first page / $4 additional** — the county's own page, not the "$26" an aggregator reports. **Caught my own error here too:** first-published totals were below the sum of their own line items; recomputed and corrected.
- **ADDENDUM (commit 2 of 2, same run) — a late research return corrected two things, including one I had just shipped.** Reported here rather than quietly amended.
  - **I attributed the wrong MUD to ShadowGlen.** Commit 1 stated ShadowGlen sits in "the Wilbarger Creek MUDs" at **$0.7720/$100**. That rate is real and correctly sourced, but it belongs to **Wilbarger Creek MUD No. 1, which covers only ~300 acres — the Laurel Trace and Stonebridge neighborhoods.** The **main ShadowGlen district is Travis County MUD No. 2 at $0.8310/$100** (2025 adopted; M&O $0.2185 + debt $0.6125), **with $0.9360 proposed for 2026** — a 10.5-cent increase the district's own notice puts at ~$239/yr on its sample home. Wilbarger Creek MUD No. 2 is **$0.9500**. Corrected in **5 places incl. FAQPage JSON-LD + MortgageBroker description**, and the finding is now *stronger* than what I first published: **"the ShadowGlen MUD rate" has no single answer — three districts, a 5.9-cent spread inside one subdivision, and the biggest one is mid-increase.** Borrowing-power math redone at $0.8310 → **~$235/mo, ~$37,250** (was $219/$34,600). Source: [Travis County Truth-in-Taxation](https://www.traviscountytx.gov/tax-rates).
  - **The Whisper Valley PID range was affirmatively wrong, not merely stale** — and it was pre-existing, carried by this page for multiple rounds. "$1,480.46–$2,003.66" appears **zero times** in the governing documents. The current instrument is the **2026 Annual Service Plan Update**, approved by Austin City Council **2026-07-23** (Ordinance 20260723-013), setting installments due 1/31/2027 across **13 lot types ranging $716.00–$2,896.49** — the old range understated the top by ~$900/yr. Also **dropped the "$55–$70/month" EcoSmart fee** for **~$60/month**: the $70 figure in press coverage was a resident's peak *summer electric bill*, not the fee. Page now tells buyers to ask which **lot type** they're in, since the type-1-to-type-13 spread is over $2,100/yr (~$175/mo of payment).
  - **Resolved an apparent conflict rather than picking a side.** The county dataset shows Manor ISD's 2026 rate as *not adopted* (voter-approval $1.0755), which appeared to contradict commit 1. The county file was last saved **2026-08-14**, three days before the board's **08-17** adoption — so the BoardBook minutes are simply newer, and the district adopted exactly at its voter-approval rate. Cross-checked the component split for internal consistency (M&O 0.7113→0.7054, debt 0.3701 unchanged). **$1.0755 stands.**
  - **Added:** City of Manor **$0.853700** and Travis County ESD 12 **$0.100000** (2025 adopted) — the city line is the second-largest component after the ISD and applies only inside city limits, not the ETJ. Plus a timing warning: as of 2026-08-20 only **4 of 146** Travis County jurisdictions had adopted a 2026 rate, with all expected by **October 3**, so a September closing may escrow against a non-final rate.
- Verification: **4/4 JSON-LD valid**, HTML tag-balanced (section 7/7, p 40/40, li 94/94, div 56/56), **FAQ 6 Q ↔ 6 accordion items in sync**, title 61 chars, meta 143 chars, **0 stale `2026-06-30`**, `dateModified` + sitemap `lastmod` → 2026-08-20. **18/20 new source URLs return HTTP 200**; the 2 exceptions (Pacesetter, CommercialSearch) 403 to curl but fetched fine in-session — bot-blocking, not dead links. Trust strip and testimonial untouched (Adam-gated). Dev-server preview blocked in unattended runs; render verification is against the live Netlify deploy.
...
- **The `8.0 months of supply` / "buyer's market" framing was load-bearing and is no longer true.** Supply ran **5.9 months on 475 active listings in April 2026** ([Neuhaus/Unlock MLS](https://neuhausre.com/liberty-hill-real-estate-market-update-may-2026/)) — back inside the 6-month balanced line. That claim appeared in **6 places, two of them inside JSON-LD** (`FAQPage` median-price answer + new-construction answer, `MortgageBroker` description, body, leverage paragraph, Step 4). All six corrected in sync. Added **Williamson County at 4.3 months on a $415,000 median, July 2026** ([Unlock MLS](https://www.unlockmls.com/news/july-2026-central-texas-housing-report)) as the current proxy, since Neuhaus discontinued the Liberty Hill series after April.
- **Differentiated insight — the MUD spread, not the rate, is the biggest lever in Liberty Hill.** New H3 section. **North San Gabriel MUD #1 adds $0.950000/$100** in Lariat ([Lariat fact sheet](https://lariatlibertyhill.com/wp-content/uploads/2025/04/Lariat-Fact-Sheet_v10.pdf)) — more than **double the entire City of Liberty Hill rate of $0.469407** — against **Clearwater Ranch at 1.727171%** (no MUD) and **Butler Farms at 2.1966%**. That is a ~92bp spread inside one city and one ISD. On a $400,000 valuation the MUD line alone is **$3,800/yr ≈ $317/mo**, or ~$50,000 of borrowing power at a stated 6.5% assumption. Replaced the page's previous hand-wave, "ESD/MUD overlays: vary by parcel."
- **Refused to publish the fact sheet's headline tax total.** It prints **2.651843%** but its own components sum to **2.696172%** — the stated total silently drops the FM-RD line (0.044329). It also carries the pre-Prop A LHISD rate (1.226900) and the prior county rate (0.399999), both superseded. Published only the MUD component, with all three defects disclosed on the page.
- **Found that Lariat's tax breakdown carries no City of Liberty Hill line**, indicating it sits outside city limits. This connects to a structural fact new to the page: **~16,000 people live inside city limits vs ~87,000 in the ETJ** ([Community Impact, May 2026](https://communityimpact.com/austin/leander-liberty-hill/development/2026/05/20/retail-follows-rooftops-after-housing-boom-stores-crop-up-along-liberty-hill-leander-georgetown-line/)). Most Liberty Hill addresses are not in the city, which is why the MUD/city tax split behaves the way it does. Framed as the organizing idea for the page.
- **3 new neighborhood spotlights, all grep-confirmed new sitewide** (`Oak Blossom`, `Butler Farms`, `Clearwater Ranch`, `Bold Sundown`, `Dove Crest`, `Louine Noble`, `North San Gabriel MUD`, `Landsea`, `Loop 332`, `2.651843`, `1.727171`, `2.1966` all returned **0 prior hits**):
  1. **Lariat (78642)** — off **CR 214 between SH-29 and the north fork of the San Gabriel River**, community address **396 Oak Blossom Rd**; **688 acres, 1,600 homes**, Phase 1 = 1,063 homesites, 45/50/60-ft lots; Ashton Woods **$328,740–$402,740** (1,466–2,653 sqft, 109 Travis Lane), Tri Pointe from the $300s (256 Faulkner Street), Perry from the $360s, plus Chesmar, Landsea, Coventry, Highland, Drees, CastleRock; HOA **$840/yr billed quarterly at $210**. Disambiguated from the similarly-named *Lariat Trails Elementary* already on the page.
  2. **Butler Farms (78642)** — the page's lowest entry point. Meritage **Americana from $283,990** (1,247–2,040 sqft) at 101 Dove Crest Lane, Boulevard $349,990, Reserve $324,990–$384,990 w/ **$41/mo HOA**; Saratoga from the low $300s at 501 Dove Crest Cove; 40–45-ft first-phase lots. The only Liberty Hill new-construction doorway clearing a **3.5% FHA down payment under $10,000**.
  3. **Clearwater Ranch (78642)** — **600+ acres** off CR 200 at **Bold Sundown**, customs **$900K–$1M+** from Drees/Giddens/Sitterle, Giddens QMIs **$1,098,000–$1,246,500** up to 4,929 sqft, **$30/mo HOA**, 12-acre creekside park. All jumbo territory, but the **lowest tax rate on the page** because there is no MUD.
- **3 new schools with cited ratings** — **Louine Noble Elementary 7/10** (101 Loop 332, PK-5, 736 students), **Liberty Hill Elementary 7/10** (1400 Loop 332, 734 students, G&T, 88% attendance vs 81% state), **Liberty Hill Middle 7/10** (13125 W SH-29, 1,050 students), all direct from GreatSchools. Flagged two naming traps for buyers: Louine Noble still indexes under its former name *Liberty Hill Intermediate*, and the middle school appears as "Liberty Hill J High School" in GreatSchools vs "Liberty Hill Middle School" in district material — same campus.
- **4 new employer/economy items** — **Santa Rita Ranch Center**: Provident's **400,000 sqft, ~$100M** center at Ronald Reagan Blvd & SH-29 anchored by a **176,000-sqft Walmart Supercenter, ~400 jobs, 2027**, plus LongHorn/Olive Garden/Ross/PetSmart ([Community Impact](https://communityimpact.com/austin/leander-liberty-hill/business/2026/05/15/walmart-supercenter-coming-to-new-santa-rita-ranch-center/)) — noting the center sits **inside a MUD, so the city collects no sales tax on it**. **Ally Medical ER** at 10280 W. Hwy. 29, winter 2026, the town's first ER. **Meridell Achievement Center (~210)** and **Divine Savior Academy (~60)** as non-construction anchors, explicitly labeled directional because the EDC's data is June 2023 Lightcast via CAPCOG. **Santa Rita Ranch is <50% built out at ~5,000 homes and sold 483 in 2025.**
- **2 templated paragraphs de-templated — and the spec's own method would have missed both.** Exact-match grep across the 24 other suburb pages returns only the shared footer and trust strip, i.e. "clean." Normalizing city names and numerics to placeholders before `difflib` similarity exposed **Step 2 "Upload pay stubs…" verbatim on 7 pages** and **Step 3 "Within one to two business days…" verbatim on 6**. Both rewritten Liberty Hill-specific: Step 2 now covers the MUD/PID notice, HOA resale certificate, well/septic and survey, and one-time-close builder docs; Step 3 covers parcel-level qualification and the **$571,550 FHA / $832,750 conforming** thresholds. Post-edit rescan: **0 templated body dupes**, body blocks 50 → 66.
- **Closing-cost examples rebuilt with primary-source math, replacing an uncited estimate.** Two worked examples: **Butler Farms FHA 3.5% at $283,990** (down $9,940, UFMIP $4,796 financed → $278,846) and **conventional 5% at the $491,049 median**. Owner's title computed off the [TDI 2026 promulgated schedule](https://tdi.texas.gov/title/titlerates2026.html) — **$1,689** and **$2,712** respectively — and I verified the formula directly against TDI's own worked example rather than reusing prior-run precedent. Homeowners insurance now cites the **Williamson County average of $2,758** ([TDI](https://www.tdi.texas.gov/general/texas-homeowners-insurance-market-overview.html), 2025 preliminary) in place of an uncited "~$3,000"; deliberately did **not** use the $3,506 statewide figure, which is inflated ~27% by Gulf Coast wind exposure. Recording at Williamson County's published **$25 first page / $4 additional**.
- Added the **2026 FHA limit for Williamson County, $571,550** ([HUD CY2026](https://www.hud.gov/pub/chums/cy2026-forward-limits.txt)) — absent from this page entirely, and the threshold most Liberty Hill resale actually falls under.
- **Dropped a citation that would not verify**: the Sitterle Homes model page returns 403, so its 216 Bold Sundown detail was cut rather than shipped behind an unverifiable link. Also declined to publish Bar W Ranch (mailing ZIP is 78641, not 78642), Liberty Parke (no fetchable builder source), Legacy Ranch Middle (no rating published yet), and Clearwater Ranch's "1-acre lots" (aggregator-only).
- Verification: **4/4 JSON-LD blocks valid**, HTML tag-balanced, meta description 163 → **153 chars**, title 60 chars, **23/24 new source URLs return HTTP 200** (the 24th cut). `dateModified` + sitemap `lastmod` → 2026-08-19. Dev-server preview is blocked in unattended runs, so render verification is against the live Netlify deploy after push.
...
- **Page arrived 100% unique at paragraph level.** A paragraph-level dedup scan across all 24 other suburb pages returned **zero cross-page body dupes** — only the shared footer and the Adam-gated trust strip matched. De-template was therefore met per R4/R5 precedent by rewriting the most template-flavored paragraph, not by cutting a dupe that did not exist.
- **4 first-party cited elements added, all grep-confirmed new sitewide** (`Hutto Highlands`, `Mager Lane`, `Innovation Blvd`, `Kval`, `Steven Scott`, `Highlands North`, `2.1625`, `319,900`, `Hutto Elementary`, `Opus Development` all returned zero prior hits anywhere in the repo):
  1. **Hutto Highlands (78634) neighborhood spotlight** — established resale at the **northeast corner of Limmer Loop and FM 1660**, building 2006 through 2023; earlier builders CastleRock, D.R. Horton, Gehan, Lennar, Scott Homes, Steven Scott, with Saratoga Homes active more recently and the *Highlands North* expansion adding Ashton Woods and Meritage; **1,508–3,313 sqft on 0.15–0.33 acre lots** ([Austin Real Estate Homes Blog](https://www.austinrealestatehomesblog.com/hutto/hutto-highlands/)); listings **$319,900–$385,000 at ~$166/sqft** on 1,612–3,064 sqft plans ([Neighborhoods.com](https://www.neighborhoods.com/hutto-highlands-hutto-tx)); no community pool and an HOA running **under $250 a year**.
  2. **Hutto Elementary 7/10, direct-fetched from GreatSchools** — 100 Mager Lane, PK–5, 819 students ([GreatSchools](https://www.greatschools.org/texas/hutto/3722-Hutto-Elementary-School/)).
  3. **Two employers** — **Kval**, advanced door-machinery manufacturer in a **52,000 sqft plant at 1591 Innovation Blvd** in Hutto's Innovation Business Park, the company's first expansion outside California, built for **80+ positions** ([Community Impact](https://communityimpact.com/pflugerville-hutto/development/groundbreaking-planned-for-new-manufacturing-site-in-hutto/)); and **Opus Development's $125M, ~800,000 sqft light-industrial** project on 69 acres in northeast Hutto fronting Ed Schmidt Blvd, unanimously advanced by City Council **May 7, 2026** ([Community Impact](https://communityimpact.com/austin/pflugerville-hutto/government/2026/05/08/125m-development-project-proposed-for-hutto/)).
  4. **A closing-cost example with real Texas numbers — FHA 3.5% down on a $319,900 Hutto Highlands resale, mid-August close**: $11,197 down, base loan $308,703, 1.75% UFMIP ($5,402) financed → **$314,105** balance; owner's title policy **$1,866** computed off the [TDI 2026 promulgated schedule](https://tdi.texas.gov/title/titlerates2026.html) effective 2026-03-01 (($319,900 − $100,000) × 0.00494 = $1,086, + $780); Williamson County recording at the cited **$25 first page / $4 each additional** ([County Clerk fees](https://www.wilcotx.gov/1613/Fees)) for ~$110–$150 on deed plus deed of trust; origination, appraisal, survey, escrow/credit/flood/tax service, a 3-month tax escrow (~$1,730) and first-year HOI → **~$8,400–$10,200**, less a **~$4,340 seller tax proration credit** (229 days elapsed at Hutto Highlands' 2.1625%) → **~$15,300–$17,100 to the closing table, down payment included**.
- **Differentiated insight — the school premium is inverted here.** Hutto Elementary at 7/10 out-rates every elementary the page's other neighborhoods feed (Cottonwood Creek 4/10, Howard Norman 5/10, Kerley 6/10) and pairs with Hutto Middle at 7/10 for the city's strongest elementary-plus-middle combination — yet the neighborhood zoned to it lists **cheaper** than Carmel Creek, Riverwalk, or an Emory Crossing new build. That inverts the usual Austin-metro pattern where the top-rated zone carries the premium, and it makes Hutto Highlands the page's best FHA and first-time doorway. Requires joining a price source to a schools source, which no competitor page does.
- **Correctness fix — the page carried a stale school rating and stale enrollment.** It stated Hutto Middle at **6/10 GreatSchools on 1,107 students, 15:1 ratio, 38% math / 53% reading**. GreatSchools direct now reads **7/10 on 744 students**. Corrected both, added the honest read that **Gus Almquist Middle opening in 2024 absorbed part of the zone** and is the likeliest driver of both changes, and told buyers an older Hutto Middle rating handed over by a listing agent is stale. **Cut the 38% / 53% proficiency pair** rather than pair a freshly verified rating with percentages that could no longer be confirmed at source.
- **Correctness — nominal vs effective tax rate.** The page quotes Hutto's ~1.93%–2.00% *effective* rate, but effective rates bake in homestead caps accrued over years of ownership and a first-year purchase carries no cap. Hutto Highlands' posted **2025 total rate was 2.1625%**, above even the $2.005 city + ISD + county nominal stack — a gap that usually signals an added special district. Published as an explicit WCAD-verify explainer (underwrite year one at the nominal rate, true up once the homestead exemption lands) rather than leaving two rates to silently contradict each other.
- **Flagged rather than asserted:** Neighborhoods.com gave a conflicting HOA figure ("$335 to $512, $30/mo") that could not be reconciled to a billing period against the neighborhood guide's "under $250/year". Published the annual figure with an explicit confirm-on-the-resale-certificate caveat. The Opus project was published with an explicit "entitlement-stage, no published jobs number, not counted as payroll until tenants sign" caveat rather than dressed up as employment.
- **De-template.** Rewrote the generic FHA loan card, cutting *"credit scores as low as 580"* — on **6** suburb pages before, **5** after (elgin/florence/jarrell/pflugerville/smithville remain) — and *"more flexible debt-to-income ratios than conventional loans"*, which Hutto was the only page carrying (now zero). Replaced with Hutto-specific FHA-versus-3%-conventional copy anchored on the $319,900 Hutto Highlands entry and Brooklands' sub-$300K new construction.
- Schema: `dateModified` 2026-06-28 → **2026-08-17**; sitemap `lastmod` 2026-06-28 → 2026-08-17; MortgageBroker description += Hutto Highlands and its 7/10 → 7/10 ladder; FAQPage **5 → 6** questions (cheapest-neighborhood-in-the-best-school-zone Q, mirrored in the visible accordion — 6 Q / 6 accordion items verified in sync). **4/4 JSON-LD valid.** Tags balanced (section 7/7, p 46/46, ul 10/10, li 80/80, h2 5/5, h3 4/4, h4 12/12, div 56/56); title lint pass; 0 stale `2026-06-28`. **9 new inline source URLs.** Verified on a local static server: **HTTP 200**, all 15 render assertions pass, all 3 negative assertions clean.
- **Concurrent writers: 64 dirty files, and both shared files are stale regressions.** The worktree `sitemap.xml` drops 23 lines and the worktree `CHANGELOG.md` deletes the 2026-08-16 Buda entry — the regression flagged in `368b045`. Staged **only this session's own hunks onto HEAD's blobs** via `git hash-object -w` + `git update-index --cacheinfo`; the working tree was never modified and the other session's edits are intact.

## 2026-08-16 — styer-suburb-editor-daily (Round 5 #6): Buda — Elm Grove spotlight, school-zone value join, Even Platforms + US Foods, nominal-vs-effective tax correction

- **4 first-party cited elements added, all grep-confirmed new sitewide** (`Even Platforms`, `Oyster Creek`, `Clear Springs Hollow`, `2.1845`, `Khosla`, `165 jobs`, `74,000` all returned zero prior hits; `Gehan` appeared on 3 other suburb pages but not Buda):
  1. **Elm Grove (78610) neighborhood spotlight** — just south of the FM 1626 / FM 967 intersection off **Oyster Creek Drive** (streets Clear Springs Hollow, Rosemary Hollow); ~320 homes in 3 phases, started by Green Builders in 2008 and finished by **Gehan Homes**, 1,870–3,270 sqft, backing the Garlic Creek Greenbelt and its 15 acres of open space ([Austin Real Estate Homes Blog](https://www.austinrealestatehomesblog.com/buda/elm-grove/)); listings **$369,000–$599,000 at ~$176/sqft**, avg 2,958 sqft, $240 association fee ([Neighborhoods.com](https://www.neighborhoods.com/elm-grove-buda-tx)).
  2. **Two school ratings, both direct-fetched from GreatSchools** — **Eric Dahlstrom Middle 8/10** (3600 FM 967, grades 6–8, 803 students) and **Moe & Gene Johnson High 6/10** (4260 Ranch Rd 967, grades 9–12, 2,869 students). The page previously carried proficiency percentages and a US News rank but no GreatSchools rating for either campus.
  3. **Two employers** — **Even Platforms**, which relocated its HQ to Buda on **July 22, 2026** (9,000 sqft at Tower Business Park, 1340 FM 2001; aerospace/defense advanced manufacturing; backed by Khosla Ventures, Link Ventures, Quiet Capital, Alumni Ventures; "dozens of high-skill, high-wage jobs over the next five years") per [Buda EDC](https://www.budaedc.com/news/even-platforms-selects-buda-texas-for-headquarters-relocation/); and the **US Foods expansion** the page had never quantified — 170,000 sqft, **165 jobs at an average $74,000 salary**, $120M+ investment, complete by 2027, $3M in incentives from the Buda EDC, City, Hays County, and the Texas Enterprise Fund ([Hays Caldwell EDP](https://hayscaldwelledp.com/about/news-and-publications/us-foods-to-expand-buda-facility-will-add-165-jobs-to-the-local-community)).
  4. **A closing-cost example with real Texas numbers — FHA 3.5% down on a $369,000 Elm Grove home, August close**: $12,915 down, base loan $356,085, 1.75% UFMIP ($6,231) financed → $362,316 balance; owner's title policy **$2,109** computed off the [TDI 2026 promulgated schedule](https://tdi.texas.gov/title/titlerates2026.html) effective 2026-03-01 (($369,000 − $100,000) × 0.00494 = $1,329, + $780); Hays County recording at the cited **$25 first page / $4 each additional** ([fee schedule](https://www.deeds.com/recorder/texas/hays/)); 3-month tax escrow $2,015; HOI, origination, appraisal/survey/credit/flood → **~$9,000–$10,300**, less a **~$5,013 seller tax proration credit** (227 days elapsed) → **~$16,900–$18,200 to close**.
- **Differentiated insight:** Elm Grove feeds the *same* Elm Grove Elementary (9/10) → Dahlstrom Middle (8/10) → Johnson High path as **Whispering Hollow, which starts around $440K** — roughly **$70K cheaper for the identical attendance zone**, and the page's strongest FHA / first-time doorway. Requires joining a price source to a feeder source, which no competitor page does.
- **Correctness — nominal vs effective tax rate.** The page quotes Buda's ~1.91% *effective* rate citywide, but effective rates bake in homestead caps accrued over years of ownership; a first-year purchase carries no cap. Elm Grove's posted **2025 total rate was 2.1845%**. Published as an explicit explainer (escrow is set off the nominal rate and trued up after the homestead exemption lands) rather than leaving two rates to silently contradict each other.
- **Aggregator override:** austinrealestatehomesblog listed Johnson High at 8/10; GreatSchools direct returned **6/10**. Published the direct figure, plus the honest read that Buda's west-side school advantage is real at elementary and middle but **evens out by high school** (Johnson 6/10 = Jack C. Hays 6/10).
- **De-template ×2.** Cut *"3.5% down with credit scores as low as 580"* from both the FHA loan card and the FAQPage schema — on **5** suburb pages before the cut, **4** after (elgin/florence/jarrell/smithville remain). Cut *"You get competing offers, not just one bank's answer"* from process step 2 — **2** before, **1** after (florence remains). A paragraph-level duplicate scan across all 25 suburb pages confirmed Buda's remaining body copy is otherwise **100% unique** — only shared footer boilerplate matched.
- **Cut as unverifiable (not published): the market-data refresh.** Four sources gave four irreconcilable figures for the same market — Redfin $381,990 (May 2026 monthly, currently on the page), Redfin $411K (trailing 3 months ending May 2026), Houzeo $425,000 (March 2026), and a $349,999 search snippet. redfin.com returns HTTP 403 to WebFetch, so the monthly series could not be re-read at source. Left the existing cited May 2026 Redfin figures untouched rather than churn an unverifiable number.
- Schema: `dateModified` 2026-06-27 → **2026-08-16**; FAQPage **5 → 6** questions (cheapest-neighborhood-in-a-top-school-zone Q, mirrored in the visible accordion — 6 items / 6 Q verified in sync); MortgageBroker description updated. **4/4 JSON-LD valid.**
- **Concurrent writers: 59 dirty files, and both shared files were stale regressions.** The worktree `sitemap.xml` reverts 22 `lastmod` values (including Pflugerville 2026-08-14 → 2026-06-26) and deletes the 2026-08-10 blog URL; the worktree `CHANGELOG.md` deletes the 2026-08-14 Pflugerville and 2026-08-10 entries. Both match the regression flagged in `368b045`. Staged **only this session's own hunks onto HEAD's versions** via `git hash-object -w` + `git update-index --cacheinfo` — the working tree was never modified and the other session's edits are intact.
...
## 2026-08-30 — Interactive: DEPLOYED the editorial redesign + audit to production (Adam-directed)

- **Merged and deployed PR #15 (sitewide editorial rollout) + PR #16 (full-site audit) to `main`** on Adam's "push it all to the live site." Merge preserved concurrent daily-agent work (GTM restore + orphan links) via manual conflict resolution on `buy-before-you-sell-austin.html` (both sides kept: GTM ×2 + audit nav CTA/Article image). All 168 tests green on the merged tree; deploy live-verified across every change class. PR #15 closed with explanatory comment (fast-forward deploy); PR #14 superseded.
- **Post-deploy fixes from Adam's live review (`0e33aaf`, `76b8def`):** lead-page trust chips were invisible (white-on-white — `.lead-page` editorial overrides missed `.lp-trust-chip`); hero columns rebalanced by moving what-happens-next into a `.lp-hero-aside` 3-step block in the copy column (get-preapproved + refinance-quote); style.css cache version bumped on both LPs.
- **A&D Mortgage removed from `p-and-l-mortgage-texas.html`** (11 mentions, visible + FAQ schema re-synced) per Adam — terms kept as "a representative program." Newrez on the 1099 page still pending Adam's call.
- **Review counts per Adam: 96 five-star Google** → 2 realtor updates updated (91→96), ARCHITECTURE.md table now 96 Google / 141 combined. Hero "141 published reviews" and the sitewide "140+" floor remain accurate.

## 2026-08-30 (later) — Calculator currency comma formatting (Adam-directed)

- All currency inputs across DSCR, asset-depletion, buydown, and refi-closing-cost calculators now display live thousands separators (shared `styerMoney` helper inlined per page, WRAP-pattern; caret-preserving). Every parser reading these fields strips commas first — guards the `parseFloat('400,000') → 400` failure mode. Slider↔input sync formats on set / strips on read; programmatic sets re-format. Percent/rate/year inputs untouched; payment/affordability/break-even are slider-driven and already formatted; WRAP already had it. Browser-verified per page with exact math cross-checks; 2 new regression tests (66 total green). Also added `20260830-lp2` to the editorial stylesheet-version allowlist test, which postdated yesterday's LP cache bump.

## 2026-09-09 — Tenant-to-owner buying guide + recommended SEO changes (Adam-directed)

- Published `blog/2026-09-09-buy-house-you-rent-from-landlord-texas.html`, a Texas-specific guide for tenants buying directly from their landlord. Research is grounded in current TREC buyer-representation guidance and resale form, Texas Property Code seller-disclosure rules, CFPB inspection/closing guidance, TDI title guidance, Fannie Mae rent-credit rules, and HUD/FHA landlord-tenant transaction guidance.
- Added answer-first copy, Article + FAQPage + BreadcrumbList JSON-LD, canonical/Open Graph/Twitter metadata, internal links, source-labeled scenario CTAs, and registrations in `blog/manifest.json`, the `blog.html` crawler/schema lists, `sitemap.xml`, and `llms.txt`.
- Reworked `new-braunfels-mortgage-lender.html` for New Braunfels/Comal County search intent. Removed unsupported market-price, school, commute, lender-count, response-time, credit-score, minimum-down, superlative, and synthetic testimonial claims; aligned the visible FAQs with FAQPage schema; added conditional scenario-first language and explicit source attribution.
- Shortened the K-1 page title and meta description and synchronized social metadata to improve the observed high-impression/low-click search opportunity.
