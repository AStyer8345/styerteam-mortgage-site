## 2026-07-17 — AI assistant production review (Claude): transcript integrity, cross-page memory, lead-flow repair

- Three-pass review of the mortgage assistant found the conversation layer healthy but the transcript pipeline lossy: sequence numbers saturated at 17/18 (capped prior-turn window ×2 formula), confirmed actions always dropped by the LoanOS idempotency cache (reused turn-1 key), and page reloads broke recording entirely (LoanOS conversation upsert rewrote the PK onto the unique session-hash row → FK violation). All confirmed empirically in Supabase before fixing.
- Website fixes (commits `674f6f1`, `5e4fab7` → main `f97263a`, `eea5225`): widget reports a monotonic `turnCount`; server derives sequences from it; conversations persist in sessionStorage across page navigation; prior turns re-redacted before reaching OpenAI; greeting + fallbacks return suggested replies; fallbacks skip already-answered questions; "your rates" recommends the rate review page; Freddie 45% DTI knowledge scoped to manual underwriting per Guide 5401.2 (manifest v3). Asset version `20260717-conversation-v3`.
- Guarded the concurrent sales-director work: goal choices that answer a topical question (e.g. "buy" after a self-employed question) stay with the knowledge path (`6e6749b` → main `b8f62d5`), with a regression test.
- LoanOS (branch `codex/website-ai-assistant`, commits `2ffec62`, `c496593`): record_conversation_turn resolves conversations by session hash without PK rewrite and appends sequences after the current max; `todo_items_source_check` widened via migration (production leads were failing at follow-up task creation — captured in runtime logs); contract priority `normal` mapped to `medium`. Deployed to production (`loanos-self.vercel.app`).
- Live scripted evaluation added at `scripts/assistant-live-eval.mjs` (15 conversational cases + optional `--lead` full lead-flow case). All cases pass against production on both domains; lead saves end-to-end; transcripts verified gap-free in Supabase; test artifacts cleaned up.
- NOTE: the chatbot's LoanOS backend now runs on the production LoanOS deployment + production Supabase (`uuqedsvjlkeszrbwzizl`) — a cutover made by the parallel Codex session this same night; staging (`qalzpfxhrafanmdunwxl`) data preserved untouched. Owner-notification email on new leads currently fails (visitor is told honestly; likely Resend/env issue) — needs Adam.
